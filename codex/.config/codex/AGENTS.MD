...
- Store working plans in `/Users/trymveiby/.config/codex/tmp/plans` so saved plans can be resumed later.
- When implementing a planned task, immediately mark that task as `done` in the saved plan.
## Skills
